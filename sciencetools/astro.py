...
def plot_images(images, column, dr, aspect=3, dpi=100, title=None, cmap="inferno"):
    ...
    fig, axes = plt.subplots(
        l, 3, figsize=(15, 15 / aspect * math.ceil(l / 3)), dpi=dpi
    )
    axs = axes.flatten()
    meds = []
    j = 0
...
def find_crop(image):
    def score(mask, x1, x2, y1, y2):
        ...
        frame = np.array(
            np.logical_and(
                np.logical_and(o[0, :, :] > x1, o[0, :, :] < x2),
                np.logical_and(o[1, :, :] > y1, o[1, :, :] < y2),
            )
        )  # IN

        return (
            -np.logical_and(mask, frame).sum()
            - np.logical_and(np.logical_not(mask), np.logical_not(frame)).sum()
        )
    ...
    params = opt.minimize(
        f,
        (50, s[0] - 50, 50, s[1] - 50),
        bounds=((0, s[0] / 2), (s[0] / 2, s[0]), (0, s[1] / 2), (s[1] / 2, s[1])),
        method="Powell",
    )
...
def make_master(frames, column, targetExptime=None, exptimeLabel="EXPTIME"):
    if targetExptime is None:
        ...
        return np.median(
            np.array(list(frames[column]))
            * targetExptime
            / np.array(list(frames["EXPTIME"]))[:, np.newaxis, np.newaxis],
            axis=0,
        )
...
def reduce(
    ...
    flatExptime,
    darkExptime,
    exptimeLabel="EXPTIME",
):
    if type(images) == pd.DataFrame:
        S = np.array(list(images[old_column]))
        ts = np.array(images[exptimeLabel])
        noemer = (
            master_flat - master_bias - master_dark * (flatExptime / darkExptime)
        )[np.newaxis, :, :]
        I = (
            S
            - master_bias[np.newaxis, :, :]
            - master_dark[np.newaxis, :, :]
            * (ts[:, np.newaxis, np.newaxis] / darkExptime)
        ) / (noemer / noemer.max())
